...
def generate_seo_and_intro(payload: dict[str, Any], page_type: str) -> dict[str, str]:
    ...
    if page_type == "university":
        ...
        target_fields = ["seo_title", "meta_description", "specialization_hero_description", "certificate_description"]
        rules = (
            ...
            "- certificate_description: 1-2 sentences describing the certificate/degree awarded for this "
            "specialization — mention it's issued by the university and is recognized/valid, similar to "
            "an on-campus degree. No markdown, plain text only.\n"
        )
    else:  # course
